...
**初めて読む方・AI向け:** プロジェクトの仕組み、用語、Gemma 4 E2B研究の到達点と未達条件を一冊にまとめた[日本語の現状説明書](docs/OPEN_TERNARY_GUIDE.ja.md)から読むと、以下の進捗記録を追いやすくなります。
...
**Eighteen-target follow-up (2026-09-25):** Layer-3 `o_proj`, which had failed in combination with `down_proj` on the earlier base, passed after the saved layer-4 additions. The 18-target hard-G128 artifact combination passed matched eager-attention BF16 gates on v4 and v5 after reload; an independent v5 repeat reproduced its summary and responses. This remains 18/205 canonical projections, with 187 in BF16. See the [research record](docs/research/gemma4-fixed-signed-hadamard-20260925.md).
...
**Layer-7 follow-up (2026-09-25):** The seven-projection addition to the saved 28-target base failed v4 instruction quality. Four individual additions passed v4, but all four together failed. A saved/reloaded `q_proj`/`k_proj`/`up_proj` 31-target candidate passed v4 and failed v5 twice on instruction quality. The other v4-passing triple, `q_proj`/`k_proj`/`v_proj`, failed a v5 in-memory screen on English PPL. The strongest saved candidate remains 28/205; see the [research record](docs/research/gemma4-fixed-signed-hadamard-20260925.md).
