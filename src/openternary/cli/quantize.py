"""quantize subcommand — Phase 0 stub."""
...
def quantize_callback(
    ...
    """Quantize model to ternary (Phase 0 stub)."""
    if ctx.invoked_subcommand is not None:
        return
    overrides = _resolve_cli_overrides(seed, device, dtype, output, group_size)
    if model is not None:
        overrides["model.id"] = model
    _handle_command("quantize", config, overrides, dry_run, verbose)
