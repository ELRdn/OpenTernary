"""inspect subcommand — Phase 0 stub."""
...
def inspect_cmd(
    ...
    """Inspect model architecture (Phase 0 stub)."""
    if ctx.invoked_subcommand is not None:
        return
    # model 引数は将来利用、Phase 0 では config の model.id を使う
    overrides = _resolve_cli_overrides(seed, device, dtype, output)
    if model is not None:
        overrides["model.id"] = model
    _handle_command("inspect", config, overrides, dry_run, verbose)
